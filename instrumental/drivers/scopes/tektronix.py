...
class Channels(object):
    ...
    def __getitem__(self, key):
        if key not in self.valid_nums:
            raise ValueError(
                "Invalid channel {}. Must be in {!r}".format(key, self.valid_nums)
            )
...
class TekScope(Scope, VisaMixin):
    ...
    def _initialize(self):
        if self.interface_type == InterfaceType.asrl:
            ...
            self._rsrc.read_termination = terminator.replace("CR", "\r").replace(
                "LF", "\n"
            )
    ...
    def get_data(self, channel=1, width=2, bounds=None):
        ...
        if bounds is None:
            ...
            if not (1 <= start <= stop <= wfm_len):
                raise ValueError(
                    "bounds must satisfy 1 <= start <= stop <= {}".format(wfm_len)
                )
    ...
    def _read_curve(self, width):
        with self.resource.ignore_warning(
    ...
    def read_measurement_stats(self, num):
        ...
        if not self.are_measurement_stats_on():
            raise Exception(
                "Measurement statistics are turned off, " "please turn them on."
            )
        ...
        res = self.query(
            prefix + ":value?;mean?;stddev?;minimum?;maximum?;count?;units?"
        ).split(";")
    ...
    def measure(self, channel, meas_type):
        ...
        msg = self.query(
            ":measu:immed:source1 ch{};type {};value?;units?"
...
class TDS_2000(TekScope):
    ...
    _INST_VISA_INFO_ = (
        "TEKTRONIX",
        ["TDS 2002B", "TDS 2004B", "TDS 2012B", "TDS 2014B", "TDS 2022B", "TDS 2024B"],
    )
...
class TDS_7000(TekScope):
    ...
    _INST_PARAMS_ = ["visa_address"]
    _INST_VISA_INFO_ = ("TEKTRONIX", ["TDS7154", "TDS7254", "TDS7404"])
...
class MSO_DPO_2000(StatScope):
    ...
    def set_min_window(self, width):
        ...
        if mantissa <= 2:
            scale_s = 0.2 * 10.0**exponent
        elif mantissa <= 4:
            scale_s = 0.4 * 10.0**exponent
        else:
            scale_s = 10.0**exponent
        scale_s = max(2e-9, scale_s)
        scale_s = min(100.0, scale_s)
