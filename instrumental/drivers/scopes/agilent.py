...
class AgilentScope(Scope, VisaMixin):
    ...
    points_mode = SCPI_Facet(
        ":WAVeform:POINts:MODE",
        convert=PointsMode,
        doc="sets the type of data returned by the :WAV:DATA? query",
    )
    points = SCPI_Facet(
        ":WAVeform:POINts", convert=int, doc="Number of points to return on :WAV:DATA?"
    )

    format = SCPI_Facet(":WAVeform:FORMat", doc="Format of the returned data")
    ...
    def capture(self, channel):
        ...
        if fmt == 1:
            ...
            dtype = "B"
        if fmt >= 0 and fmt <= 1:
            data = self._rsrc.query_binary_values(
                ":WAVeform:DATA? CHAN%i" % channel, datatype=dtype, is_big_endian=True
            )
