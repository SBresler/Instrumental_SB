...
def ret_error(error):
    if error != 0:
        ...
        if bool(NicePicamLib.IsLibraryInitialized()):
            msg = NicePicamLib.GetEnumerationString(
...
def ret_enum_string_error(error):
    if error != 0:
        ...
        if error == lib.PicamError_EnumerationValueNotDefined:
            ...
            msg = "Error when getting enumeration string. Error code {}".format(
                code.name
            )
...
class PicamRois(object):
    ...
    def __init__(self, rois_ptr):
        self._ptr = rois_ptr
        self._rois = [
            PicamRoi(self, rois_ptr.roi_array[i]) for i in range(rois_ptr.roi_count)
        ]
...
class PicamRoi(object):
    ...
    def __repr__(self):
        return (
            f"PicamRoi({self.x=}, {self.y=}, {self.width=}, {self.height=}, "
...
class PicamModulations(object):
    ...
    def __init__(self, mods_ptr):
        self._ptr = mods_ptr
        self._mods = [
            PicamModulation(self, mods_ptr.modulation_array[i])
...
class PicamModulation(object):
    ...
    def __repr__(self):
        return (
            f"PicamModulation({self.duration=}, {self.frequency=}, {self.phase=}, "
...
def _find_camera_id(paramset):
    ...
    try:
        ...
        if not (
            paramset.get("is_demo") and "model" in paramset and "serial" in paramset
        ):
            raise
...
class EnumerationParameter(IntegerParameter):
    def __init__(self, dev, parameter):
        ...
        etype = PicamEnums.EnumeratedType(
            self._dev.GetParameterEnumeratedType(self._param)
        )
...
class PicamCamera(Camera):
    ...
    def start_capture(self, **kwds):
        ...
        self.set_roi(
            x=int(kwds["left"]),
            y=int(kwds["top"]),
            width=int(kwds["width"]),
            height=int(kwds["height"]),
            x_binning=kwds["hbin"],
            y_binning=kwds["vbin"],
        )
    ...
    def wait_for_frame(self, timeout=None):
        ...
        try:
            self._latest_available_data, _ = self._dev.WaitForAcquisitionUpdate(
                timeout_ms
            )
        except PicamError as e:
    ...
    def _extract_available_data(self, available_data, copy=True):
        ...
        all_data = _c_data_to_numpy(
            available_data.initial_readout, size, dtype=np.uint16
        )

        rois = self.params.Rois.get_value()
        roi_shapes = [
            (roi.width // roi.x_binning, roi.height // roi.y_binning) for roi in rois
        ]
    ...
    def set_roi(
        self, x=None, y=None, width=None, height=None, x_binning=None, y_binning=None
    ):
    ...
    def _get_rois_constraint(self):
        ...
        return ffi.gc(
            rois_constraint, ignore_error(NicePicamLib.DestroyRoisConstraints)
        )
    ...
    def commit_parameters(self):
        ...
        if n > 0:
            bad_str = ",".join(
                PicamEnums.Parameter(bad_params[i]).name for i in range(n)
            )
            raise PicamError(
                "{} parameters were unsuccessfully committed: [{}]".format(n, bad_str)
            )
