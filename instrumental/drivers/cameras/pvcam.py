...
class PVCam(Camera):
    ...
    def _try_uninit():
        try:
            ...
            if e.err_code not in (pv.C0_PVCAM_NOT_INITED, pv.C2_PVCAM_NOT_INITED):
                raise e
    ...
    def wait_for_frame(self, timeout=None):
        ...
        while (
            status != pv.READOUT_COMPLETE
            and status != pv.READOUT_FAILED
            and (timeout is None or t <= tfinal)
        ):
    ...
    def setup_sequence(self, exp_time="100ms", nframes=1, mode="timed", regions=None):
        ...
        pv.exp_setup_seq(
            self.hcam, n_exposures, len(self.regions), region_p, mode, exp_time, size_p
        )

        # Create buffer
        hbuf_p = ffi.new("int16_ptr")
        pv.buf_alloc(
            hbuf_p, n_exposures, pv.PRECISION_UNS16, len(self.regions), region_p
        )
        self.hbuf = hbuf_p[0]
    ...
    def grab_frame(self, fresh_capture=True, force_list=False, timeout=-1):
        ...
        t = time.clock()
        tfinal = float("inf") if timeout == -1 else (t + timeout / 1000.0)
        status = pv.READOUT_NOT_ACTIVE
        while (
            status != pv.READOUT_COMPLETE
            and status != pv.READOUT_FAILED
    ...
    def grab_ndarray(self, fresh_capture=True, force_list=False, timeout=-1):
        ...
        for i, per_exposure_bufs in enumerate(bufs):
            ...
            for buf, region in zip(per_exposure_bufs, self.regions):
                ...
                per_exposure_arrs.append(
                    np.frombuffer(buffer(buf), np.uint16, w * h).reshape((h, w))
                )
