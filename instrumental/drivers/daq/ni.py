...
class NiceNI(NiceLib):
    ...
    class Task(NiceObject):
        ...
        ReadDigitalLines = Sig(
            "in", "in", "in", "in", "arr", "len=in", "out", "out", "ignore"
        )
...
class ValEnumMeta(EnumMeta):
    ...
    def __new__(metacls, cls, bases, clsdict, **kwds):
        ...
        return super(ValEnumMeta, metacls).__new__(
            metacls, cls, bases, enum_dict, **kwds
        )
...
_internal_channels = {
    ...
    ProductCategory.USBDAQ: ["_cjtemp", "_cjtemp0", "_cjtemp1", "_cjtemp2", "_cjtemp3"],
...
class Task(object):
    ...
    def set_timing(
        ...
        if num_args_specified == 0:
            ...
            self.fsamp, self.n_samples = handle_timing_params(
            ...
            for dev_mtasks in self._mtasks.values():
                for ch_type, mtask in dev_mtasks.items():
                    if clock is not None:
                        ...
                        ch_clock = (
                            self.master_clock if ch_type != self.master_type else ""
                        )
                    mtask.config_timing(
                        self.fsamp, self.n_samples, mode, self.edge, ch_clock
                    )
        else:
    ...
    def read(self, timeout=None):
        timeout_s = float(-1.0 if timeout is None else timeout.m_as("s"))
        ...
        for ch_name, ch in self.channels.items():
            if ch.type in ("AO", "DO", "CO"):
                if write_data is None:
                    ...
                    raise ValueError(
                        "write_data missing an array for output channel {}".format(
                            ch_name
                        )
                    )
    ...
    def stop(self):
        ...
        """
        self._mtasks[self.master_device][
            self.master_type
        ].stop()  # Stop the master first
        for devname, dev_mtasks in self._mtasks.items():
            for ch_type, mtask in dev_mtasks.items():
                if not (ch_type == self.master_type and devname == self.master_device):
                    mtask.stop()
    ...
    def _read_AI_channels(self, timeout_s):
        ...
        if is_scalar:
            ...
            end_t = (
                (n_samps_read - 1) / self.fsamp.m_as("Hz")
                if self.fsamp is not None
                else 0
            )
            res["t"] = Q_(np.linspace(0.0, end_t, n_samps_read), "s")
        return res
    ...
    def _write_AO_channels(self, data, autostart=True):
        ...
        for dev_name, dev_mtasks in self._mtasks.items():
            ...
            ao_names = [
                ...
                if ch.type == "AO" and ch.daq.name == dev_name
            ]
            arr = np.concatenate(
                [Q_(data[ao]).to("V").magnitude for ao in ao_names]
            ).astype(np.float64)
            n_samps_per_chan = len(list(data.values())[0].magnitude)
            mx_task.WriteAnalogF64(
                n_samps_per_chan, autostart, -1.0, Val.GroupByChannel, arr
            )
...
def enum_property(name, enum_type, doc=None):
    return mk_property(
        name, conv_in=lambda x: as_enum(enum_type, x).value, conv_out=enum_type, doc=doc
    )
...
class MiniTask(object):
    ...
    def config_timing(self, fsamp, n_samples, mode="finite", edge="rising", clock=""):
        ...
        self._mx_task.CfgSampClkTiming(
            clock, fsamp.m_as("Hz"), edge.value, mode.value, n_samples
        )
    ...
    def _assert_io_type(self, io_type):
        if io_type != self.io_type:
            raise TypeError(
                "MiniTask must have io_type '{}' for this operation, but is of "
    ...
    def add_AI_channel(self, ai, term_cfg="default", vmin=None, vmax=None):
        ...
        self._mx_task.CreateAIVoltageChan(
            ai_path, "", term_cfg.value, vmin.m_as("V"), vmax.m_as("V"), Val.Volts, ""
        )
    ...
    def add_AO_channel(self, ao):
        ...
        self._mx_task.CreateAOVoltageChan(
            ao_path, "", min.m_as("V"), max.m_as("V"), Val.Volts, ""
        )
    ...
    def read_AI_channels(self, samples=-1, timeout=None):
        ...
        data, n_samples_read = self._mx_task.ReadAnalogF64(
            samples, timeout_s, Val.GroupByChannel, buf_size
        )
        res = {}
        ...
        res["t"] = Q_(
            np.linspace(
                0,
                n_samples_read / self.fsamp.m_as("Hz"),
                n_samples_read,
                endpoint=False,
            ),
            "s",
    ...
    def read_DI_channels(self, samples=-1, timeout=None):
        ...
        res = self._mx_task.ReadDigitalU32(
            samples, timeout_s, Val.GroupByChannel, buf_size
        )
        ...
        if self.fsamp is not None:
            ...
            res["t"] = Q_(np.linspace(0.0, end_t, n_samples_per_chan_read), "s")
        return res
    ...
    def write_AO_channels(self, data, timeout=-1.0, autostart=True):
        ...
        arr = np.concatenate([data[ao].m_as("V") for ao in self.chans]).astype(
            np.float64
        )
        n_samples = len(list(data.values())[0].magnitude)
        self._mx_task.WriteAnalogF64(
            n_samples, autostart, timeout, Val.GroupByChannel, arr
        )
...
class AnalogIn(Channel):
    ...
    def _add_to_minitask(self, minitask, term_cfg="default"):
        ...
        mx_task.CreateAIVoltageChan(
            self.path, "", term_cfg.value, min.m_as("V"), max.m_as("V"), Val.Volts, ""
        )
    ...
    def read(
        ...
        with self.daq._create_mini_task("AI") as mtask:
            ...
            if num_args_specified == 0:
                ...
                raise ValueError(
                    "Must specify 0 or 2 of duration, fsamp, and n_samples"
                )
        return data
    ...
    def start_reading(
        ...
        vmax=None,
        overwrite=False,
        relative_to=RelativeTo.CurrReadPos,
        offset=0,
        buf_size=10,
    ):
...
class AnalogOut(Channel):
    ...
    def _add_to_minitask(self, minitask):
        min, max = self.daq._max_AO_range()
        mx_task = minitask._mx_task
        mx_task.CreateAOVoltageChan(
            self.path, "", min.m_as("V"), max.m_as("V"), Val.Volts, ""
        )
        ...
        with self.daq._create_mini_task("AI") as mtask:
            ...
            try:
                ...
                raise NotSupportedError(
                    "DAQ model does not have the internal channel required "
                    "to sample the output voltage"
                )

            num_args_specified = sum(
                int(arg is not None) for arg in (duration, fsamp, n_samples)
            )
    ...
    def write(
        self, data, duration=None, reps=None, fsamp=None, freq=None, onboard=True
    ):
...
class VirtualDigitalChannel(Channel):
    ...
    def _add_to_minitask(self, minitask, split_lines=False):
        ...
        if self.type == "DI":
            ...
            raise TypeError(
                "Can't add digital channel to MiniTask of type {}".format(io_type)
            )
    ...
    def read(self, duration=None, fsamp=None, n_samples=None):
        ...
        with self.daq._create_mini_task("DI") as minitask:
            ...
            if num_args_specified == 0:
                ...
                raise ValueError(
                    "Must specify nothing, fsamp only, or two of duration, fsamp, "
                    "and n_samples"
                )
        return data
...
class NIDAQ(DAQ):
    ...
    product_category = property(
        lambda self: ProductCategory(self._dev.GetDevProductCategory())
    )
