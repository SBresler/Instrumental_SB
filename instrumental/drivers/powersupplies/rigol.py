...
class DP700(RigolPowerSupply, VisaMixin):
    voltage = SCPI_Facet(
        "SOURce:VOLTage:LEVel:IMMediate:AMPLitude", convert=float, units="V"
    )
    current = SCPI_Facet(
        "SOURce:CURRent:LEVel:IMMediate:AMPLitude", convert=float, units="A"
    )
    current_protection = SCPI_Facet(
        "SOURce:CURRent:PROTection", convert=float, units="A"
    )
    current_protection_state = SCPI_Facet(
        "SOURce:CURRent:PROTection:STATe", convert=OnOffState
    )
