...
class SR850:
    ...
    def set_output_offsets_and_expands(self, offset_selector, offset, expand):
        ...
        command_string = command_string.format(
            offset_selector.value, offset, int(expand)
        )
    ...
    @check_enums(trace=TraceNumber, m1=Multiply, m2=Multiply, d=Divide, store=Store)
    def set_trace_definitions(
        self, trace, m1, m2=Multiply.unity, d=Divide.unity, store=Store.stored
    ):
        ...
        command_string = command_string.format(trace, m1, m2, d, store)
    ...
    def get_trace(self, trace_number, points=None, units=None, binary=True):
        ...
        command_string = "{}? {}, {}, {}".format(
            command, trace_number.value, points[0], points[1]
        )
        if binary:
            ...
            self._rsrc.timeout = (
                2 * BINARY_TIME_PER_POINT.to("ms").magnitude * points[1] + timeout
            )
            ...
            with self._rsrc.ignore_warning(visa.constants.VI_SUCCESS_MAX_CNT):
                raw_binary, _ = self._rsrc.visalib.read(
                    self._rsrc.session, points[1] * BYTES_PER_POINT
                )
            ...
            # Factor of 2 is so that the transfer completes before timing out
            self._rsrc.timeout = (
                2 * ASCII_TIME_PER_POINT.to("ms").magnitude * points[1] + timeout
            )
            value = self._rsrc.query(command_string)
    ...
    def _set(self, command_string, unit_string, value):
        value = Q_(value)
        command_string = "{} {}".format(command_string, value.to(unit_string).magnitude)
        self._send_command(command_string)
