...
with open(
    os.path.join(os.path.dirname(__file__), "_filter_flipper", "FilterFlipper.h")
) as f:
    ffi.cdef(f.read())
...
def list_instruments():
    ...
    return [
        ParamSet(Filter_Flipper, serial=serial_str)
...
error_dict = {
    ...
    38: (
        "INVALID_POSITION - The function cannot be performed as it would result in an illegal "
        "position."
    ),
