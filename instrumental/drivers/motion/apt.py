...
def list_instruments():
    return [
        ParamSet(TDC001_APT, port=p.device)
        for p in comports()
...
class TDC001_APT(APT):
    ...
    def get_jog_params(self):
        ...
        jog_step_size = Q_(
            struct.unpack("<i", rsp[10:14])[0] * self.pos_scaling_factor, self.pos_units
        )
        jog_accel = Q_(
            struct.unpack("<i", rsp[18:22])[0] * self.vel_scaling_factor, self.vel_units
        )
        jog_max_accel = Q_(
            struct.unpack("<i", rsp[22:26])[0] * self.acc_scaling_factor, self.acc_units
        )
    ...
    def move_to(self, position):
        ...
        cmd = bytes(
            [0x53, 0x04, 0x06, 0x00, self.dst | 0x80, self.src, 0x01, 0x00]
        ) + struct.pack("<i", int(position.magnitude / self.pos_scaling_factor))
        self._ser.write(cmd)
        return
